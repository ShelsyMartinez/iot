...
@router.get("/mqtt-acl")
async def obtener_acl(usuario: dict = Depends(get_current_user)):
    """
    Devuelve la lista de ACL (reglas por usuario) para tu dashboard.
    """
    try:
        # Este endpoint es para EMQX v4.x y v5.x
        resp = await emqx_get("/emqx_acl")
        # resp puede tener structure {data:[...]} o direct array
        return resp.get("data", resp)
    except httpx.HTTPError as e:
        raise HTTPException(status_code=500, detail=f"Error al obtener ACL: {e}")
